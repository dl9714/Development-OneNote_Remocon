...
def ensure_pywinauto():
    ...
    try:
        ...
        print("[ERROR] pywinauto를 가져올 수 없습니다. 설치되어 있는지 확인하세요.")
        # 이 경우, 일부 기능(연결 및 스크롤)은 작동하지 않음.
        # 실행 자체는 가능하도록 _pwa_ready = True로 설정하지 않습니다.
...
class OtherWindowSelectionDialog(QDialog):
    def __init__(self, my_pid: int, parent=None):
        ...
        # ... (Dialog UI 및 로직은 변경 없음, 상위 코드를 참고) ...
...
class OneNoteScrollRemoconApp(QWidget):
    ...
    def init_ui(self, initial_status):
        ...
        self.btn_add_section_other = QToolButton()  # Placeholder for context menu
        ...
        tb1_layout.addWidget(self.btn_add_group)
        ...
        tb2_layout.addWidget(self.btn_delete)
        ...
        self.connection_status_label.setObjectName("StatusBarLabel")
        main_layout.addWidget(self.connection_status_label)

        self.setLayout(main_layout)
        splitter.setSizes([280, 680])

    # ----------------- 14.1 창 닫기 이벤트 핸들러 (Geometry 저장) -----------------
    def closeEvent(self, event):
        self._save_window_geometry()
        self._save_favorites()  # 마지막으로 즐겨찾기 상태 저장
    ...
    # ----------------- 15. 즐겨찾기 로드/세이브 (통합 설정 파일 사용) -----------------
    def _load_favorites(self):
        self.fav_tree.clear()

        # __init__에서 로드된 self.settings 사용
    ...
    def _save_favorites(self):
        try:
            ...
            # 전체 설정을 다시 로드하여 덮어쓰기 방지 (특히 connection_signature)
            current_settings = load_settings()
    ...
    # ... (즐겨찾기 serialize, append, 조작 로직은 동일) ...
    ...
    def _activate_favorite_section(
        ...
        if not _pwa_ready:
            QMessageBox.warning(self, "오류", "자동화 모듈이 로드되지 않았습니다.")
            return
        ...
        if connected and self._auto_center_after_activate:
            ...
            if "onenote" in exe_name or "onenote" in (sig.get("title") or "").lower():
                ...
                if section_text:
                    ...
                    if ok:
                        # 0.5초 지연 후 중앙 정렬 실행 (선택 반영 시간 고려)
                        QTimer.singleShot(
                            500,
        ...
        # 연결 상태는 _perform_connection에서 업데이트되므로, 여기서 중복 업데이트 방지
