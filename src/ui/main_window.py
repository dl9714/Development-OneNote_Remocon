...
"""
    ...
    def _codex_selected_package_codex_skills(self) -> List[str]:
        ...
        if skill_list is not None:
            for item in skill_list.selectedItems():
    ...
    def _current_codex_skill_package(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": desc,
            "user_skills": self._codex_text_lines_from_widget(
                getattr(self, "codex_skill_package_user_skills_editor", None)
            ),
...
"""
    ...
    def _build_codex_skill_package_group(self) -> QWidget:
        ...
        self.codex_skill_package_user_skills_editor = QTextEdit()
        self.codex_skill_package_user_skills_editor.setMinimumHeight(70)
        self.codex_skill_package_user_skills_editor.setPlaceholderText(
            "사용자 스킬 주문번호를 한 줄에 하나씩 입력\n예:\nSK-001\nSK-002"
        )
        self.codex_skill_package_user_skills_editor.textChanged.connect(
            self._schedule_codex_skill_package_preview
        )
        form.addWidget(QLabel("사용자 스킬"), 2, 0)
        form.addWidget(self.codex_skill_package_user_skills_editor, 2, 1)
        ...
        for name in self._codex_skill_package_default_codex_skills():
            self.codex_skill_package_codex_skill_list.addItem(name)
        self.codex_skill_package_codex_skill_list.itemSelectionChanged.connect(
            self._schedule_codex_skill_package_preview
        )
    ...
    def _show_onenote_harness_help(self) -> None:
        ...
        help_view.setPlainText(
            """원노트 하네스

스킬 패키지
사용자 스킬들을 조합한 템플릿입니다. 자주 쓰는 글쓰기 방식, 에이전트 역할, 실행 작업을 하나의 구성으로 묶어 Codex 요청을 빠르게 만들 수 있습니다.
...
코덱스 지침
Codex가 OneNote 작업을 안전하게 실행하기 위한 내부 실행 기준입니다.
...
1. 사용할 사용자 스킬을 고릅니다.
2. 실행할 코덱스 스킬을 고릅니다.
3. 대상 위치, 제목, 본문 같은 요청 내용을 입력합니다.
4. Codex는 코덱스 지침을 기준으로 실행하고 검증합니다."""
        )
