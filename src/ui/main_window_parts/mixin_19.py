    ...
    def _codex_skill_package_default_instructions(self) -> List[str]:
        ...
        if _codex_active_platform_key() == CODEX_PLATFORM_MACOS:
            return common + [
                ...
                "대량 페이지 이동은 하나의 AppleScript 루프로 묶고 첫 이동 후 다시 이동 메뉴를 재사용",
                "이동 시트의 대상 row는 캐시하되 실패 시 시트를 정리하고 한 번만 재스캔",
                "같은 제목이 대상 섹션에 있어도 원본 실제 글은 이동하고 중복 후보는 삭제 전 확인",
                "페이지 제목 변경은 메뉴의 페이지 이름 바꾸기를 우선 사용하고 목록 제목으로 검증",
                "한글 제목 입력 누락 시 ASCII 접두어 입력 후 클립보드 붙여넣기로 보강",
            ]
        return common + [
